...
int jtag_main( int argc, char **argv) {
	...
	printf("index num=%u\n",index_num);
	if(vjtag_init( )<1) {
		fprintf (stderr, "Error inititaing virtual jtag tab\n");
		return -1;
		
	}
	if (jtag_open_virtual_device(index_num)<1){
...
int send_binary_file( ){
	...
	jtag_vdr(BIT_NUM, memory_offset_in_word, &out);
	...
	for(i=0;i<num;i++){
		...
		if(BYTE_NUM < sizeof(unsigned )){
			jtag_vdr(BIT_NUM, buffer[i],  &out);
		}else {
			reorder_buffer(&buffer[i],words);
			jtag_vdr_long(BIT_NUM, &buffer[i],  &out, words);
			i+= (words-1);
...
int vdr_large (unsigned sz, char * string, char *out){
	...
	if( out == 0) return  jtag_vdr_long(sz,val,val_o,words);
