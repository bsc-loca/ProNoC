...
void detect_merge(char *s1, char * s2){
	if( (strlen(s1) + strlen(s2) +1) >1000) return;
...
detect_parts( chain_t *chain, const char *db_path )
{
	...
	sprintf( out, _("IR length: %d\n"), irlen );
	detect_merge( chain->output, out); 
	chain->total_instr_len = irlen;

	/* Allocate IR */
	ir = register_fill( register_alloc( irlen ), 1 );
	if (ir == NULL) {
		sprintf( out, _("out of memory\n") );
		detect_merge( chain->output, out); 
		return 0;
	}
	...
	if (chlen < 1) {
		sprintf( out, _("Unable to detect JTAG chain length\n") );
		detect_merge( chain->output, out); 
		return 0;
	}
	sprintf( out, _("Chain length: %d\n"), chlen );
	detect_merge( chain->output, out); 
	...
	for (i = 0; i < chlen; i++) {
		...
		if (!find_record( data_path, key, &idr )) {
			sprintf( out, _("  Unknown manufacturer!\n") );
			detect_merge( chain->output, out); 
			register_free( key );
			continue;
		}
		...
		sprintf( out, _("  Manufacturer: %s\n"), idr.fullname );
		detect_merge( chain->output, out); 
		if (strlen( idr.fullname ) > MAXLEN_MANUFACTURER)
			sprintf( out, _("Warning: Manufacturer too long\n") );
			detect_merge( chain->output, out); 
		...
		if (!find_record( data_path, key, &idr )) {
			sprintf( out, _("  Unknown stepping!\n") );
			detect_merge( chain->output, out); 
			register_free( key );
			continue;
		}
		...
		sprintf( out, _("  Stepping:     %s\n"), idr.fullname );
		detect_merge( chain->output, out); 
		if (strlen( idr.fullname ) > MAXLEN_STEPPING)
			sprintf( out, _("Warning: Stepping too long\n") );
			detect_merge( chain->output, out); 
	...
	for (i = 0; i < 32; i++) {
		tap_shift_register( chain, one, br, EXITMODE_SHIFT );
		if (register_compare( one, br ) != 0) {
			sprintf( out, _("Error: Unable to detect JTAG chain end!\n") );
			detect_merge( chain->output, out); 
			break;
		}
	}
...
int manual_add(chain_t *chain, int instr_len)
{
	...
	id = register_alloc( 1 );
	if (id == NULL) {
		sprintf( out, _("Error: Unable to allocate a register!\n") );
		detect_merge( chain->output, out); 
		return 0;
	}
	...
	if (chain->parts == NULL) {
		...
		if (chain->parts == NULL) {
			sprintf( out, _("Error: Unable to allocate space for parts!\n") );
			detect_merge( chain->output, out); 
			return 0;
		}
	}
	...
	part = part_alloc(id);
	if (part == NULL) {
		sprintf( out, _("Error: Unable to allocate space for a part!\n") );
		detect_merge( chain->output, out); 
		return 0;
	}
	...
	if (cmd_run(chain, cmd) < 1) {
		sprintf( out, _("Error: could not set BR register") );
		detect_merge( chain->output, out); 
		return 0;
	}
	...
	if (result < 1) {
		sprintf( out, _("Error: could not set BYPASS instruction") );
		detect_merge( chain->output, out); 
		return 0;
	}
