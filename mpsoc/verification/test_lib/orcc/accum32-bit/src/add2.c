// Source file is "L/add/src/accumulatorv2/accumulator2.cal"
...
////////////////////////////////////////////////////////////////////////////////
// Input FIFOs
extern fifo_i32_t *add2_Input1;
extern fifo_i32_t *add2_Input2;
...
#define rate_Input1 connection_add2_Input1.rate
...
#define rate_Input2 connection_add2_Input2.rate
...
static void read_Input1() {
	index_Input1 = add2_Input1->read_inds[0];
	numTokens_Input1 = index_Input1 + fifo_i32_get_num_tokens(add2_Input1, 0);
}

static void read_end_Input1() {
	add2_Input1->read_inds[0] = index_Input1;
}
static void read_Input2() {
	index_Input2 = add2_Input2->read_inds[0];
	numTokens_Input2 = index_Input2 + fifo_i32_get_num_tokens(add2_Input2, 0);
}

static void read_end_Input2() {
	add2_Input2->read_inds[0] = index_Input2;
}

static void write_Output() {
	index_Output = add2_Output->write_ind;
}

static void write_end_Output() {
	add2_Output->write_ind = index_Output;
}
...
void add2_initialize(schedinfo_t *si) {
	int i = 0;
	write_Output();
finished:
	write_end_Output();
...
////////////////////////////////////////////////////////////////////////////////
// Action scheduler
void add2_scheduler(schedinfo_t *si) {
	int i = 0;
	si->ports = 0;
	...
	while (1) {
		if (numTokens_Input1 - index_Input1 >= 1 && numTokens_Input2 - index_Input2 >= 1 && isSchedulable_add()) {
			int stop = 0;
			if (1 > SIZE_Output - index_Output + add2_Output->read_inds[0]) {
				stop = 1;
			}
			if (stop != 0) {
				si->num_firings = i;
