...
/**********************************************************************
**	File: /home/alireza/work/hca_git/ProNoC/mpsoc/src_topolgy/muli/muli_noc_genvar.v
**    
...
module   muli_noc_genvar #(  
	...
	for( i=0; i<3; i=i+1) begin : router_2_port_lp
	...
	(	
		...
		.flit_in_all( router_flit_in_all [i][(2 * Fw)-1    :   0]),
		.flit_out_all( router_flit_out_all [i][(2 * Fw)-1    :   0]),
		.flit_in_wr_all( router_flit_in_wr_all [i][(2 * 1)-1    :   0]),
		.flit_out_wr_all( router_flit_out_wr_all [i][(2 * 1)-1    :   0]),
		.congestion_in_all( router_congestion_in_all [i][(2 * CONGw)-1    :   0]),
		.congestion_out_all( router_congestion_out_all [i][(2 * CONGw)-1    :   0]),
		.credit_out_all( router_credit_out_all [i][(2 * V)-1    :   0]),
		.credit_in_all( router_credit_in_all [i][(2 * V)-1    :   0])
	);

    
	end    
			
	for( i=0; i<2; i=i+1) begin : router_3_port_lp
	...
	(	
		...
		.current_r_addr(i+3),
		.neighbors_r_addr({RAw{1'b0}}),
		.flit_in_all( router_flit_in_all [i+3][(3 * Fw)-1    :   0]),
		.flit_out_all( router_flit_out_all [i+3][(3 * Fw)-1    :   0]),
		.flit_in_wr_all( router_flit_in_wr_all [i+3][(3 * 1)-1    :   0]),
		.flit_out_wr_all( router_flit_out_wr_all [i+3][(3 * 1)-1    :   0]),
		.congestion_in_all( router_congestion_in_all [i+3][(3 * CONGw)-1    :   0]),
		.congestion_out_all( router_congestion_out_all [i+3][(3 * CONGw)-1    :   0]),
		.credit_out_all( router_credit_out_all [i+3][(3 * V)-1    :   0]),
		.credit_in_all( router_credit_in_all [i+3][(3 * V)-1    :   0])
	);

    
	end    
			
	for( i=0; i<1; i=i+1) begin : router_4_port_lp
	...
	(	
		...
		.current_r_addr(i+5),
		.neighbors_r_addr({RAw{1'b0}}),
		.flit_in_all( router_flit_in_all [i+5][(4 * Fw)-1    :   0]),
		.flit_out_all( router_flit_out_all [i+5][(4 * Fw)-1    :   0]),
		.flit_in_wr_all( router_flit_in_wr_all [i+5][(4 * 1)-1    :   0]),
		.flit_out_wr_all( router_flit_out_wr_all [i+5][(4 * 1)-1    :   0]),
		.congestion_in_all( router_congestion_in_all [i+5][(4 * CONGw)-1    :   0]),
		.congestion_out_all( router_congestion_out_all [i+5][(4 * CONGw)-1    :   0]),
		.credit_out_all( router_credit_out_all [i+5][(4 * V)-1    :   0]),
		.credit_in_all( router_credit_in_all [i+5][(4 * V)-1    :   0])
	);
...
//Connect R0 input ports 1 to  R3 output ports 1
		assign  router_flit_in_all [0][(2*Fw)-1 :	 1*Fw ] = router_flit_out_all [3][(2*Fw)-1 :	 1*Fw ];
		assign  router_flit_in_wr_all [0][1] = router_flit_out_wr_all [3][1];
		assign  router_congestion_in_all [0][(2*CONGw)-1 :	 1*CONGw ] = router_congestion_out_all [3][(2*CONGw)-1 :	 1*CONGw ];
		assign  router_credit_in_all [3][(2*V)-1 :	 1*V ] = router_credit_out_all [0][(2*V)-1 :	 1*V ];
...
//Connect R1 input ports 1 to  R5 output ports 2
		assign  router_flit_in_all [1][(2*Fw)-1 :	 1*Fw ] = router_flit_out_all [5][(3*Fw)-1 :	 2*Fw ];
		assign  router_flit_in_wr_all [1][1] = router_flit_out_wr_all [5][2];
		assign  router_congestion_in_all [1][(2*CONGw)-1 :	 1*CONGw ] = router_congestion_out_all [5][(3*CONGw)-1 :	 2*CONGw ];
		assign  router_credit_in_all [5][(3*V)-1 :	 2*V ] = router_credit_out_all [1][(2*V)-1 :	 1*V ];
...
//Connect R2 input ports 1 to  R5 output ports 3
		assign  router_flit_in_all [2][(2*Fw)-1 :	 1*Fw ] = router_flit_out_all [5][(4*Fw)-1 :	 3*Fw ];
		assign  router_flit_in_wr_all [2][1] = router_flit_out_wr_all [5][3];
		assign  router_congestion_in_all [2][(2*CONGw)-1 :	 1*CONGw ] = router_congestion_out_all [5][(4*CONGw)-1 :	 3*CONGw ];
		assign  router_credit_in_all [5][(4*V)-1 :	 3*V ] = router_credit_out_all [2][(2*V)-1 :	 1*V ];
...
//Connect R3 input ports 1 to  R0 output ports 1
		assign  router_flit_in_all [3][(2*Fw)-1 :	 1*Fw ] = router_flit_out_all [0][(2*Fw)-1 :	 1*Fw ];
		assign  router_flit_in_wr_all [3][1] = router_flit_out_wr_all [0][1];
		assign  router_congestion_in_all [3][(2*CONGw)-1 :	 1*CONGw ] = router_congestion_out_all [0][(2*CONGw)-1 :	 1*CONGw ];
		assign  router_credit_in_all [0][(2*V)-1 :	 1*V ] = router_credit_out_all [3][(2*V)-1 :	 1*V ];
...
//Connect R4 input ports 1 to  R5 output ports 1
		assign  router_flit_in_all [4][(2*Fw)-1 :	 1*Fw ] = router_flit_out_all [5][(2*Fw)-1 :	 1*Fw ];
		assign  router_flit_in_wr_all [4][1] = router_flit_out_wr_all [5][1];
		assign  router_congestion_in_all [4][(2*CONGw)-1 :	 1*CONGw ] = router_congestion_out_all [5][(2*CONGw)-1 :	 1*CONGw ];
		assign  router_credit_in_all [5][(2*V)-1 :	 1*V ] = router_credit_out_all [4][(2*V)-1 :	 1*V ];
...
//Connect R5 input ports 1 to  R4 output ports 1
		assign  router_flit_in_all [5][(2*Fw)-1 :	 1*Fw ] = router_flit_out_all [4][(2*Fw)-1 :	 1*Fw ];
		assign  router_flit_in_wr_all [5][1] = router_flit_out_wr_all [4][1];
		assign  router_congestion_in_all [5][(2*CONGw)-1 :	 1*CONGw ] = router_congestion_out_all [4][(2*CONGw)-1 :	 1*CONGw ];
		assign  router_credit_in_all [4][(2*V)-1 :	 1*V ] = router_credit_out_all [5][(2*V)-1 :	 1*V ];
//Connect R5 input ports 2 to  R1 output ports 1
		assign  router_flit_in_all [5][(3*Fw)-1 :	 2*Fw ] = router_flit_out_all [1][(2*Fw)-1 :	 1*Fw ];
		assign  router_flit_in_wr_all [5][2] = router_flit_out_wr_all [1][1];
		assign  router_congestion_in_all [5][(3*CONGw)-1 :	 2*CONGw ] = router_congestion_out_all [1][(2*CONGw)-1 :	 1*CONGw ];
		assign  router_credit_in_all [1][(2*V)-1 :	 1*V ] = router_credit_out_all [5][(3*V)-1 :	 2*V ];
//Connect R5 input ports 3 to  R2 output ports 1
		assign  router_flit_in_all [5][(4*Fw)-1 :	 3*Fw ] = router_flit_out_all [2][(2*Fw)-1 :	 1*Fw ];
		assign  router_flit_in_wr_all [5][3] = router_flit_out_wr_all [2][1];
		assign  router_congestion_in_all [5][(4*CONGw)-1 :	 3*CONGw ] = router_congestion_out_all [2][(2*CONGw)-1 :	 1*CONGw ];
		assign  router_credit_in_all [2][(2*V)-1 :	 1*V ] = router_credit_out_all [5][(4*V)-1 :	 3*V ];
