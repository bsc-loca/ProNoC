...
addr=0;
 ...
 pck_injct_trace_file[1] =1'b1;
    repeat (REPEAT_NUM)begin 
	addr = addr+  CACHE_BLK_SIZ;	
	gen_rntxn(1'b0, 1'b0, addr, REQ_OPCODE_CleanUnique, 1); 
...
initial begin
	#25000
	file = $fopen("/home/alireza/work/hca_git/ProNoC/mpsoc/src_coherency/inj-verify/wr_rd_rd_clean2/trace/ReadShared.bin", "rb");
	@(posedge pck_injct_trace_file[0]) #1	
	pck_inject_rd_trace(2*REPEAT_NUM);
	$fclose(file);
	#10
	//file = $fopen("/home/alireza/work/hca_git/ProNoC/mpsoc/src_coherency/inj-verify/wr_rd_rd_clean2/trace/ReadUnique.bin", "rb");
