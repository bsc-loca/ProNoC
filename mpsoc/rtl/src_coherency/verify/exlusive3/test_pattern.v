...
************");
 ...
 addr=0;
 repeat(REPEAT_NUM)begin
	//1-Readshared Exlusive all rns
	addr = addr+  CACHE_BLK_SIZ;	
	gen_rntxn(1'b1, 1'b1, addr, REQ_OPCODE_ReadShared, 0); 
	gen_rntxn(1'b1, 1'b1, addr, REQ_OPCODE_ReadShared, 1); 
	gen_rntxn(1'b1, 1'b1, addr, REQ_OPCODE_ReadShared, 2); 
	gen_rntxn(1'b1, 1'b1, addr, REQ_OPCODE_ReadShared, 3); 
        #4100;  
 
	
 
	//2- CleanUnique Exlusive all rna only one must sucecss
        gen_rntxn(1'b1, 1'b1, addr, REQ_OPCODE_CleanUnique, 0);
	gen_rntxn(1'b1, 1'b1, addr, REQ_OPCODE_CleanUnique, 1); 
	gen_rntxn(1'b1, 1'b1, addr, REQ_OPCODE_CleanUnique, 2); 
	gen_rntxn(1'b1, 1'b1, addr, REQ_OPCODE_CleanUnique, 3);  
