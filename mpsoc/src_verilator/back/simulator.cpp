...
int  HOTSPOT_NUM;
int  HOTSPOT_CORE_1, HOTSPOT_CORE_2, HOTSPOT_CORE_3, HOTSPOT_CORE_4,HOTSPOT_CORE_5,HOTSPOT_SEND;

void  usage(){
	printf(" ./simulator -f [Traffic Pattern file]\n\nor\n");


	printf(" ./simulator -t [Traffic Pattern]  -s  [PACKET SIZE]  -n  [MAX_PCK_NUM]  c	[MAX SIM CLKs]   -i [INJECTION RATIO] -p [class traffic ratios (%%)]  -h[HOTSPOT info] \n");
...
void processArgs (int argc, char **argv )
{
   ...
   opterr = 0;
   if (argc < 2)  usage();	
   while ((c = getopt (argc, argv, "t:s:n:c:i:p:h:f:")) != -1)
      {
	 switch (c)
	    {
	 	case 'f':
	 		...
	 		load_traffic_file(optarg);
	 		MAX_PCK_NUM=total_pck_num_custom;
	 		break;
	    case 't':  
			TRAFFIC=optarg;
			break;
...
void pck_dst_gen_2D (
	...
	if((strcmp (TRAFFIC,"RANDOM")==0) || (strcmp (TRAFFIC,"random")==0)){
	   ...
	   (*dest_x) = (rnd % NX );
		...
		else if((HOTSPOT_NUM > 1)	&& (rnd100 >= (1*max_percent))	&& (rnd100 < ((1*max_percent)+HOTSPOT_PERCENTAGE)) && core_num!=HOTSPOT_CORE_2 )  rnd = HOTSPOT_CORE_2;
		else if((HOTSPOT_NUM > 2)	&& (rnd100 >= (2*max_percent))	&& (rnd100 < ((2*max_percent)+HOTSPOT_PERCENTAGE)) && core_num!=HOTSPOT_CORE_3 )  rnd = HOTSPOT_CORE_3;
		else if((HOTSPOT_NUM > 3)	&& (rnd100 >= (3*max_percent))	&& (rnd100 < ((3*max_percent)+HOTSPOT_PERCENTAGE)) && core_num!=HOTSPOT_CORE_4 )  rnd = HOTSPOT_CORE_4;
		else if((HOTSPOT_NUM > 4)	&& (rnd100 >= (4*max_percent))	&& (rnd100 < ((4*max_percent)+HOTSPOT_PERCENTAGE)) && core_num!=HOTSPOT_CORE_5 )  rnd = HOTSPOT_CORE_5;

		if(HOTSPOT_SEND==0){
			if(core_num ==HOTSPOT_CORE_1) rnd = core_num;
			else if((HOTSPOT_NUM > 1)	&& (core_num ==HOTSPOT_CORE_2)) rnd = core_num;
			else if((HOTSPOT_NUM > 2)	&& (core_num ==HOTSPOT_CORE_3)) rnd = core_num;
			else if((HOTSPOT_NUM > 3)	&& (core_num ==HOTSPOT_CORE_4)) rnd = core_num;
			else if((HOTSPOT_NUM > 4)   && (core_num ==HOTSPOT_CORE_5)) rnd = core_num;
		}
...
unsigned int pck_dst_gen_custom ( unsigned int src){
	
	
	int loc = active_location[src];
	if(loc == DISABLE){
		 return src; //disable sending
	} 
    if(TrafficTable[loc].src != src){
		
		 return src; //disable sending
	}
	if( TrafficTable[loc].packet_sent == 0){
		
		traffic[src]->pck_size_in=TrafficTable[loc].avg_pck_size;
		traffic[src]->ratio=TrafficTable[loc].pir * MAX_RATIO/100;
		traffic[src]->pck_class_in=  pck_class_in_gen(src);
		traffic[src]->init_weight=TrafficTable[loc].initial_weight;		
	}
	
    if( TrafficTable[loc].packet_sent >=  TrafficTable[loc].total_pck_num){
		active_location[src]=active_location[src]+1;
		loc++;
		
		if(TrafficTable[loc].src != src){	
			printf("%d disable sending\n",src);	
			traffic[src]->ratio=0;
			return src; //disable sending
	...
	TrafficTable[loc].packet_sent=TrafficTable[loc].packet_sent +1;
	return TrafficTable[loc].dst;
...
void pck_dst_gen (
	...
	if(TRAFFIC_TYPE==CUSTOM){
		int dest = 	pck_dst_gen_custom ( core_num);
		 (*dest_y) = (dest / NX );
		 (*dest_x) = (dest % NX );	
