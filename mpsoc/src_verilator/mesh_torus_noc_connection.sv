		...
		if( TOPOLOGY == "RING" || TOPOLOGY == "LINE") begin : ring_line 
			...
			for  (x=0;   x<NX; x=x+1) begin :Router_
				...
				if(x    <   NX-1) begin: not_last_node            
					assign  router_chan_out[x][FORWARD] = router_chan_in [(x+1)][BACKWARD];
				end else begin :last_node
					/* verilator lint_off WIDTH */ 
					if(TOPOLOGY == "LINE") begin : line_last_x
					/* verilator lint_on WIDTH */ 
						assign  router_chan_out[x][FORWARD]= {ROUTER_CHANEL_w{1'b0}};										      
					end else begin : ring_last_x
						assign router_chan_out[x][FORWARD]= router_chan_in [0][BACKWARD];
					end
				end 
            
				if(x>0)begin :not_first_x
					assign router_chan_out[x][BACKWARD]= router_chan_in [(x-1)][FORWARD];					
				end else begin :first_x
					/* verilator lint_off WIDTH */ 
					if(TOPOLOGY == "LINE") begin : line_first_x
						/* verilator lint_on WIDTH */ 
						assign  router_chan_out[x][BACKWARD]={ROUTER_CHANEL_w{1'b0}};					
					end else begin : ring_first_x
						assign  router_chan_out[x][BACKWARD]= router_chan_in [(NX-1)][FORWARD];											
					end
				end            
				...
				for  (l=0;   l<NL; l=l+1) begin :locals
					...
					assign router_chan_out[x][LOCALP]= chan_in_all [ENDPID];
					assign chan_out_all [ENDPID] = router_chan_in[x][LOCALP];
    ...
    in [x,y][south] <------ out [x   ,y+1][north] ;
				 */    
					...
					if(x    <    NX-1) begin: not_last_x
						assign router_chan_out[`router_id(x,y)][EAST]= router_chan_in [`router_id(x+1,y)][WEST];
						...
						if(TOPOLOGY == "MESH") begin :last_x_mesh
							/* verilator lint_on WIDTH */ 
							assign router_chan_out[`router_id(x,y)][EAST] = {ROUTER_CHANEL_w{1'b0}};					
							/* verilator lint_off WIDTH */ 
						end else if(TOPOLOGY == "TORUS") begin : last_x_torus
							/* verilator lint_on WIDTH */ 
							assign router_chan_out[`router_id(x,y)][EAST] = router_chan_in [`router_id(0,y)][WEST];						
						end //topology
					end 
		        
		    
					if(y>0) begin : not_first_y
						assign router_chan_out[`router_id(x,y)][NORTH] =  router_chan_in [`router_id(x,(y-1))][SOUTH];					
					end else begin :first_y
						/* verilator lint_off WIDTH */ 
						if(TOPOLOGY == "MESH") begin : first_y_mesh
						/* verilator lint_on WIDTH */ 
							assign router_chan_out[`router_id(x,y)][NORTH] =  {ROUTER_CHANEL_w{1'b0}};												
							/* verilator lint_off WIDTH */ 
						end else if(TOPOLOGY == "TORUS") begin :first_y_torus
							/* verilator lint_on WIDTH */ 
							assign router_chan_out[`router_id(x,y)][NORTH] =  router_chan_in [`router_id(x,(NY-1))][SOUTH];						
						end//topology
					end//y>0
		        
		        
					if(x>0)begin :not_first_x
						assign    router_chan_out[`router_id(x,y)][WEST] =  router_chan_in [`router_id((x-1),y)][EAST];					
					end else begin :first_x
						/* verilator lint_off WIDTH */ 
						if(TOPOLOGY == "MESH") begin :first_x_mesh
							/* verilator lint_on WIDTH */ 
							assign    router_chan_out[`router_id(x,y)][WEST] =   {ROUTER_CHANEL_w{1'b0}};						
							/* verilator lint_off WIDTH */                
						end else if(TOPOLOGY == "TORUS") begin :first_x_torus
							/* verilator lint_on WIDTH */ 
							assign    router_chan_out[`router_id(x,y)][WEST] =   router_chan_in [`router_id((NX-1),y)][EAST] ;						
						end//topology
					end    
		        
					if(y    <    NY-1) begin : firsty
						assign  router_chan_out[`router_id(x,y)][SOUTH] =    router_chan_in [`router_id(x,(y+1))][NORTH];					
					end else     begin : lasty
						/* verilator lint_off WIDTH */ 
						if(TOPOLOGY == "MESH") begin :ly_mesh
							/* verilator lint_on WIDTH */ 
							assign  router_chan_out[`router_id(x,y)][SOUTH]=  {ROUTER_CHANEL_w{1'b0}};						
							/* verilator lint_off WIDTH */ 
						end else if(TOPOLOGY == "TORUS") begin :ly_torus
							/* verilator lint_on WIDTH */ 
							assign  router_chan_out[`router_id(x,y)][SOUTH]=    router_chan_in [`router_id(x,0)][NORTH];						
						end//topology
					end          
					...
					for  (l=0;   l<NL; l=l+1) begin :locals
						...
						assign router_chan_out [`router_id(x,y)][LOCALP] =    chan_in_all [ENDPID];
						assign chan_out_all [ENDPID] = router_chan_in [`router_id(x,y)][LOCALP];
