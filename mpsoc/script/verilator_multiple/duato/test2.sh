...
cp $path/src_c/plot/plot $multiple_path/plot_bin
    ...
    CLASS_SETTING="4'b1111"  # no class. packets can be sent to any availbale OVC
...
gen_testbench_name(){
	testbench_name="$VC_REALLOCATION_TYPE"$B"_"$routename"_"$TRAFFIC"_"$PACKET_SIZE
		
}

gen_plot_name(){
	plot_name="$VC_REALLOCATION_TYPE"$B"_"$TRAFFIC"_"$PACKET_SIZE
...
for B in    8
do					
 for VC_REALLOCATION_TYPE in "NONATOMIC"  "ATOMIC"
 do 
