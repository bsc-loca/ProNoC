        ...
        if(ROUTE_TYPE ==  "FULL_ADAPTIVE") begin: full_adpt
             ...
             // The EVC can only forward the packet to another EVC
              ...
              if(  ESCAP_VC_MASK[i%V])  begin  // the sender is EVC
                 assign candidate_ovc_y_all[((i+1)*V)-1 : i*V] =  (y_evc_forbiden[i]) ? {V{1'b0}}:  // packet can not be sent to any EVC or AVC  
                ...
                assign candidate_ovc_y_all[((i+1)*V)-1 : i*V] =  (y_evc_forbiden[i]) ? candidate_ovc_all[((i+1)*V)-1 : i*V] & (~ESCAP_VC_MASK) : // // packet can not be sent to any EVC
...
************************/
    ...
    input           x_diff_is_one,current_x_0;
...
// check if EVC is allowed to be used     
    ...
    end else if (ROUTE_SUBFUNC=="NORTH_LAST") begin 
        // Using of all EVCs located in y dimention are restricted when the packet can be sent into both x& +y direction  
        assign y_evc_forbiden = a&b&y;  
        //there is no restriction in using EVCs located in x dimention
        assign x_evc_forbiden = 1'b0;  
    end else if (ROUTE_SUBFUNC=="ODD_EVEN") begin 
