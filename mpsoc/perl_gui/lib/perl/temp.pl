...
sub build_gui_old {
	...
	$hbox->attach($self->build_menu("$sw/",$window,$tree_view,$tree_store,$scwin_dirs), 0, 1, 0,1,'shrink','shrink',2,2);
...
sub build_gui {
	...
	my $hbox = def_table(FALSE, 0);
	my $source_notebook = gen_notebook();	
	$vbox->pack_start($source_notebook, FALSE, FALSE, 0);
